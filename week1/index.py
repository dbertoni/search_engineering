...
def main(source_dir: str, file_glob: str, index_name: str, workers: int, host: str, max_docs: int, batch_size: int, refresh_interval: str):
    ...
    new_settings = {"index": {"refresh_interval": refresh_interval}}
    response = client.indices.put_settings(index=index_name, body=new_settings)
    # Check the changes
    if response["acknowledged"]:
        print(f"The refresh_interval in '{index_name}' has been updated to '{refresh_interval}'")
    else:
        print(f"Error updating the refresh_interval in '{index_name}'")
    ...
    new_settings = {"index": {"refresh_interval": "5s"}}
    response = client.indices.put_settings(index=index_name, body=new_settings)
